<template>
	<view class="content" :style="{ 'padding-top': statusBarHeight + 'px' }">
		<u-navbar title="付款管理" bgColor="#fff" @leftClick="leftClick" @rightClick="rightClick">
		...
		<view class="card" v-for="(item, index) in paymentList" :key="index">
								...
								<u-row>
									<u-col span="4.5">
										<u-form-item label="付款日期">
											...
											<text>{{item.payedTime ? item.payedTime : '2022-04-01'}}</text>
										</u-form-item>
									</u-col>
									<u-col span="4.5">
										<u-form-item label="付款期次">
											<text>第{{item.period ? item.period : '1'}}期</text>
										</u-form-item>
									</u-col>
								</u-row>
								<u-row>
		...
		<u-empty v-if="paymentList.length==0 && isLoading" mode="data"
			icon="http://cdn.uviewui.com/uview/empty/data.png">
		</u-empty>
		<!-- <u-loadmore v-if="paymentList.length>0" :status="status" :loading-text="loadingText" :loadmore-text="loadmoreText"
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				getPaymentList(data, res => {
					if (res.code == 200 && res.success) {
						console.log(res)
						if (this.pageNo == 1) {
							this.paymentList = res.result.records;
						} else {
							this.paymentList = this.paymentList.concat(res.result.records)
						}
			...
			gotoPaymentDetail(item) {
				uni.setStorageSync('paymentItem', item)
				uni.navigateTo({
					url: "/pages/workbench/accountingManagement/paymentManagement/paymentDetail"
				})
			},
			clickScreen() {
				if (!this.firstTime) {
			...
			rightClick() {
				uni.navigateTo({
					url: "/pages/workbench/accountingManagement/paymentManagement/addPaymentRecord?returnPageNumber=1"
				})
			}
		},
	}
