<template>
	<view>
		...
		<u--form :model="collectionForm" ref="form1" labelPosition="left" labelWidth="80">
			<view style="flex: auto; height:auto!important;">
				<scroll-view class="scrool-more" :style="{ 'top': statusBarHeight + 'px','bottom':'72px' }"
					...
					<view class="top-height">
						<u-form-item label="回款日期" prop="collectionForm.returnTime" ref="item1" :required="true"
							borderBottom @click="clickDatePicker(1); hideKeyboard()">
							<u--input v-model="collectionForm.returnTime" border="none" placeholder="请选择" disabled
								disabledColor="#ffffff" clearable maxlength="30"></u--input>
							<u-icon slot="right" name="arrow-right"></u-icon>
						</u-form-item>
						<u-form-item class="amount" label="回款金额(元)" prop="collectionForm.amount" :required="true"
							labelWidth="125" borderBottom>
							<u--input v-model="collectionForm.amount" border="none" placeholder="请填写" clearable
								maxlength="30" type="digit">
							</u--input>
						</u-form-item>
						<u-form-item label="关联客户" prop="collectionForm.customerName" ref="item1" borderBottom
							@click="gotorelatedCustomers(); hideKeyboard()">
							<u--input v-model="collectionForm.customerName" border="none" placeholder="请选择" disabled
								disabledColor="#ffffff" clearable maxlength="30"></u--input>
							<u-icon slot="right" name="arrow-right"></u-icon>
						</u-form-item>
						<u-form-item label="关联合同" prop="collectionForm.contractName" ref="item1" :required="true"
							borderBottom @click="gotorelatedContracts(); hideKeyboard()">
							<u--input v-model="collectionForm.contractName" border="none" placeholder="请选择" disabled
								disabledColor="#ffffff" clearable maxlength="30"></u--input>
							<u-icon slot="right" name="arrow-right"></u-icon>
						</u-form-item>
						<!-- <u-form-item label="关联合同" prop="collectionForm.contractName" :required="true" ref="item1"
							borderBottom @click="clickExpense('related_contract'); hideKeyboard()">
							<u--input v-model="collectionForm.contractName" border="none" placeholder="请选择" disabled
								disabledColor="#ffffff" clearable maxlength="30"></u--input>
							<u-icon slot="right" name="arrow-right"></u-icon>
						</u-form-item> -->
						<u-form-item label="回款期次" prop="period" ref="item1" :required="true" borderBottom
					...
					<view class="top-height">
						<u-form-item label="关联发票" prop="collectionForm.invoiceName" ref="item1" borderBottom
							@click="clickExpense(); hideKeyboard()">
							<u--input v-model="collectionForm.invoiceName" border="none" placeholder="请选择" disabled
								disabledColor="#ffffff" clearable maxlength="30"></u--input>
							<u-icon slot="right" name="arrow-right"></u-icon>
						</u-form-item>
					</view>
					<view class="top-height">
						<u-form-item class="collection_person" label="回款所属人" labelWidth="85" prop="belongingPersonnel"
					...
					<view class="top-height">
						<u-form-item label="备注信息" prop="collectionForm.remark" ref="item1"></u-form-item>
						<u--textarea v-model="collectionForm.remark" border="none" placeholder="请输入备注信息"
							maxlength="300">
						</u--textarea>
					</view>
					<view class="top-height">
						<u-form-item label="通知人员" @click="clickUser();isBelongingPersonnel=false; hideKeyboard()">
							<u--input v-model="collectionForm.userName" border="none" placeholder="请选择" disabled
								disabledColor="#ffffff" clearable maxlength="30"></u--input>
							<u-icon slot="right" name="arrow-right"></u-icon>
						</u-form-item>
					</view>
			...
			<view class="foot-height">
				<view style="padding: 0 16px;">
					<u-form-item>
						<u-button color="rgba(0, 114, 255, 1)" shape="circle" text="保存" @click="saveCollection">
						</u-button>
					</u-form-item>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			console.log(option);
			this.isEditId = option.editCollectionId || ""
			if (this.isEditId) {
				this.navTitle = "编辑回款单"
				let editData = uni.getStorageSync("editItem")
				let lstActiviti = uni.getStorageSync("lstActiviti")
				this.collectionForm = editData
				this.collectionForm.returnTime = editData.time
				...
				this.collectionForm.vars = lstActiviti
			}
		...
		onShow() {
			...
			uni.$once('returnContract', res => {
				console.log(res)
				this.collectionForm.contractName = res.contractName; //合同名称
				this.collectionForm.contractId = res.contractId; //合同ID
			...
			uni.$once('resultSelectUser', res => {
				...
				if (lstSelectUser) {
					...
					if (this.isBelongingPersonnel) {
						...
						// this.collectionForm.belongUserId = chargeId
						// this.getDepartData(this.collectionForm.belongUserId);
					} else {
						this.collectionForm.userName = "已选择" + lstSelectUser.length + "人"
		...
		methods: {
				...
				getProcessStartVariablesByBizCode("COLLECTION ", res => {
					if (res.code == 200 && res.success) {
						...
						for (let i = 0; i < res.result.length; i++) {
							...
							this.collectionForm.vars.push(info)
			...
			// 查询回款期次
			getCollectionPeriod() {
				let data = "?contractId=" + this.collectionForm.contractId +
					"&pageNo=1&pageSize=10"
				getPlanItemList(data, res => {
					if (res.code == 200 && res.success) {
			...
			clickExpense(type) {
				...
				switch (type) {
					...
					case 'cash_plan_periods': //回款期次
						if (isEmpty(this.collectionForm.contractId)) {
			...
			confirmExpense_type(e) {
				...
				switch (type) {
					case 'related_contract':
						// this.collectionForm.bizOpptySource = e.value[0].value + '';
						this.collectionForm.contractName = e.value[0].label;
						break;

					case 'cash_plan_periods':
						// this.collectionForm.transactionProbability = e.value[0].value + '';
						this.collectionForm.itemId = e.value[0].itemId;
						...
						this.collectionForm.period = e.value[0].value
						this.collectionForm.planId = e.value[0].planId;

						break;
					case 'pay_type':
						// this.collectionForm.transactionProbability = e.value[0].value + '';
						this.payType = e.value[0].label
						this.collectionForm.payType = e.value[0].value
						break;
				}
				this.showPicker = false;
			},
			...
			clickDatePicker(type) {
				...
				if (type == 1) {
					this.datetime = this.collectionForm.returnTime
					this.isreturnTime = true
					this.dateMode = 'date'
				} else {
					this.datetime = this.collectionForm.contactTask.connectTime
					this.isreturnTime = false
			...
			confirm(e) {
				...
				if (this.isreturnTime) {
					this.collectionForm.returnTime = timeFormat(e.value, 'yyyy-mm-dd');
				} else {
					this.collectionForm.contactTask.connectTime = timeFormat(e.value, 'yyyy-mm-dd hh:MM');
				}
			...
			//验证数据
			validateField() {
				if (isEmpty(this.collectionForm.returnTime)) {
					uni.$u.toast("请选择回款日期");
					return false;
				}
				if (isEmpty(this.collectionForm.amount)) {
					uni.$u.toast("请填写回款金额");
					return false;
				}
				if (isEmpty(this.collectionForm.customerId)) {
					uni.$u.toast("请选择关联合同");
					return false;
				}
			...
			saveCollection() {
				if (this.validateField() && !this.isClick) {
					let data = this.collectionForm
					console.log(data);
					if (this.isEditId) {
						editCollection(data, res => {
							if (res.code == 200 && res.success) {
								console.log(res);
								this.isClick = true;
								uni.$u.toast(res.message);
				...
				// this.collectionForm.amount = Number(this.collectionForm.amount)
				// this.collectionForm.period = this.period
