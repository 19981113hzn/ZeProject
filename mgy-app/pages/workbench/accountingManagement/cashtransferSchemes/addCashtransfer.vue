		...
		<u--form v-model="formData" ref="form1" labelPosition="left" labelWidth="120">
			...
			<view style="flex: auto; height:auto!important;">
				<scroll-view class="scrool-more" :style="{ 'top': statusBarHeight + 'px','bottom':'110px' }"
					...
					<!-- 无数据时 -->
					<!-- <view v-if="formData.collectionPlanItems.length == 0 && this.EditId" class="main-card">
						...
						<view class="header">
							<u-grid :border="false" col="2">
								...
								<u-grid-item>
									...
									<text class="grid-text" style="font-size: 13px;">总回款金额</text>
								</u-grid-item>
							</u-grid>

						</view>
						<view class="content" v-for="(item,index) in formData.collectionPlanItems" :key="index">
							<view class="prod-item">
								<u-cell customStyle="color: #303133;">
									<view slot="title">第1期</view>
									<view v-if="formData.collectionPlanItems.length>1" slot="right-icon"
										class="u-slot-value" @click="deletePlan(item,index)">
										<u-icon name="/static/img/icon_jian@2.png" size="24"></u-icon>
									</view>
								</u-cell>
								<view class="top-height">
									<u-form-item label="计划回款日期" prop="collectionTime" ref="item1" borderTop borderBottom
										labelWidth="130" :required="true"
										@click="clickDatePicker('plan_time'); hideKeyboard()">
										<u--input :data-index="index" v-model="item.collectionTime" border="none"
											placeholder="请选择" disabledColor="#ffffff" clearable maxlength="30">
										</u--input>
										<u-icon slot="right" name="arrow-right"></u-icon>
									</u-form-item>
									<u-form-item class="cashplan" prop="amount" label="计划回款金额(元)" borderBottom
										labelWidth="130" :required="true">
						...
						<view class="addCash" v-if="formData.collectionPlanItems.length > 0">
							<u-button customStyle="color: #0072FF; border:none;" icon="/static/img/icon_add.png"
								text="添加回款期数" iconColor="#0072FF" @click="addPeriods">
							</u-button>
							...
							<u-grid :border="false" col="2">
								<u-grid-item>
									<text class="grid-text"
										style="font-size: 16px;">{{formData.collectionPlanItems.length}}期</text>
									<text class="grid-text" style="font-size: 13px;">总计划期次</text>
								</u-grid-item>
								<u-grid-item>
	...
	export default {
		...
		onLoad(option) {
			...
			if (this.isEditId) {
				...
				this.formData.contractId = EditData.contractId
				...
				this.formData.collectionPlanItems = EditPlanItem
		...
		methods: {
			onInput(e) {
				let total = this.formData.collectionPlanItems.map((item, index) => {
					return item.amount
				})
				this.formData.totalAmount = total.reduce((accumulator, currentValue) => Number(accumulator) + Number(
					currentValue));
			...
			addPeriods() {
				...
				this.formData.collectionPlanItems.forEach((item) => {
				...
				if (statusType) {
					this.formData.collectionPlanItems.push({
						collectionTime: "",
						period: this.formData.collectionPlanItems.length + 1,
						amount: "",
						remark: ""
					});
				}
			...
			deletePlan(item, index) {
				this.formData.collectionPlanItems.splice(index, 1)
				let total = this.formData.collectionPlanItems.map((item, index) => {
					return item.amount
				})
				this.formData.totalAmount = total.reduce((accumulator, currentValue) => Number(accumulator) + Number(
			...
			clickDatePicker(type) {
				...
				if (type == 1) {
					...
					this.datetime = this.collectionTime
				}
			...
			confirm(e) {
				...
				this.formData.collectionPlanItems.forEach((item, index) => {
				...
				if (this.selectType == 1) {
					...
					console.log('666');
					this.formData.collectionPlanItems[this.timeIndex].collectionTime = timeFormat(e.value,
						'yyyy-mm-dd')
				}
			...
			addPlanForm() {
				if (this.validateField() && !this.isClick) {
					...
					if (this.formData.amount != this.formData.totalAmount) {
						uni.$u.toast("合同总金额与总回款金额不符")
					} else {
						if (!isEmpty(this.isEditId)) {
							editCashPlan(this.formData, res => {
