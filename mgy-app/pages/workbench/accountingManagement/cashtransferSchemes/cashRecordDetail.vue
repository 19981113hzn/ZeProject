<template>
	<view :style="{ 'padding-top': statusBarHeight + 'px' }">
		<u-navbar title="第1期回款计划" @leftClick="leftClick"></u-navbar>
		...
		<view v-if="collectionRecordLists">
			<view class="card" v-for="(item, index) in collectionRecordLists" :key="index">
				<view class="card-pa">
		...
		<view v-if="collectionRecordLists == null" class="sjd-prod">
			<view class="prod-null">
				<image class="no-logo" src="/static/img/workbench/icon_PDF.png">
				</image>
				<view class="no-text">暂无记录</view>
				<u-button type="primary" size="small" text="去添加" @click="gotoAddCollection" shape="circle"
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			let data = uni.getStorageSync('collectionPlanItems')
			this.collectionRecordLists = data
			console.log(this.collectionRecordLists);
			this.getDictItems()
			// this.getCollection()
		},
		methods: {
			//获取回款记录详情列表
			// getCollection() {
			// 	let data = '?planItemId=' + this.collectionId
			// 	getCollectionList(data, res => {
			// 		if (res.code == 200 && res.success) {
			// 			this.collectionRecordLists = res.result.records
			// 			console.log(this.collectionRecordLists);
			// 		} else {
			...
			gotoAddCollection() {
				uni.redirectTo({
					url: "/pages/workbench/accountingManagement/cashtransferSchemes/addCollectionRecord?returnPageNumber=3"
				})
			},
