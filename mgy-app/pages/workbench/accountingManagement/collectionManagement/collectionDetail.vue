<template>
	<view :style="{ 'padding-top': statusBarHeight + 'px' }">
		<u-navbar :title="collectionDetails.contractName ? collectionDetails.contractName : '合同标题'"
		...
		<view>
			<u-cell-group class="sign-out" :border="false">
				<u-cell :title="collectionDetails.createBy + '的回款申请'" size="large">
					...
					<u-tag v-if="collectionDetails.status==3" slot="value" text="已通过" bgColor="rgba(247, 252, 244, 1)"
						borderColor="rgba(247, 252, 244, 1)" color="rgba(103, 194, 58, 1)" size="mini">
					</u-tag>
					<u-tag v-if="collectionDetails.status==1 || collectionDetails.status==2" slot="value" text="待审批"
						bgColor="rgba(235, 244, 255, 1)" borderColor="rgba(235, 244, 255, 1)"
						color="rgba(0, 114, 255, 1)" size="mini">
					</u-tag>
					<u-tag v-if="collectionDetails.status==5" slot="value" text="被驳回" bgColor="rgba(255, 248, 242, 1)"
						borderColor="rgba(255, 248, 242, 1)" color="rgba(255, 147, 48, 1)" size="mini">
					</u-tag>
					<u-tag v-if="collectionDetails.status==6" slot="value" text="已撤销" bgColor="rgba(241, 245, 250, 1)"
			...
			<view class="main-crad">
				<view class="main-crad-bg">
					...
					<text class="card-name">回款金额</text>
					<text class="card-num">¥ {{collectionDetails.amount}}</text>
					<u-cell :title="collectionDetails.contractName ? collectionDetails.contractName : '-'">
						<image class="icon-slot" slot="icon" src="/static/img/workbench/icon_title.png"></image>
					</u-cell>
					<view class="card_title">
						<u-cell>
							<text slot="title">{{collectionDetails.period}}期(共{{collectionDetails.nums}}期)</text>
							<image class="icon-slot" slot="icon" src="/static/img/workbench/icon_periods.png"></image>
						</u-cell>
						<u-cell :title="collectionDetails.time ? collectionDetails.time : '2022-08-06'">
						...
						<view class="card-pa">
							...
							<u--form labelWidth="100">
								<u-form-item label="回款日期">
									<text>{{collectionDetails.time ? collectionDetails.time : '2022-08-06'}}</text>
								</u-form-item>
								<u-form-item label="回款金额">
									<text>{{collectionDetails.amount}}</text>
								</u-form-item>
								<u-form-item label="关联客户">
									<text
										style="color: #0072FF;">{{collectionDetails.customerName ? collectionDetails.customerName : '-'}}</text>
								</u-form-item>
								<u-form-item label="关联合同">
									<text
										style="color: #0072FF;">{{collectionDetails.contractName ? collectionDetails.contractName : '-'}}</text>
								</u-form-item>
								<u-form-item label="回款期次">
									<text>{{collectionDetails.period}}期(共{{collectionDetails.nums}}期)</text>
								</u-form-item>
								<u-form-item label="回款计划">
									<text style="color: #0072FF;" @click="seePlan">查看计划</text>
								</u-form-item>
								<u-form-item label="付款方式">
									<text>{{collectionDetails.payTypeName}}</text>
								</u-form-item>
								<u-form-item label="逾期状态">
									<text>{{collectionDetails.overdueStatusName}}</text>
								</u-form-item>
								<u-form-item label="发票信息">
									<text>{{collectionDetails.invoiceName ? collectionDetails.invoiceName : '-'}}</text>
								</u-form-item>
								<u-form-item label="回款所属人员">
									<text>{{collectionDetails.userName ? collectionDetails.userName : '赵小刚'}}</text>
								</u-form-item>
								<u-form-item label="创建人员">
									<text>{{collectionDetails.createBy ? collectionDetails.createBy : '赵小刚'}}</text>
								</u-form-item>
							</u--form>
						</view>
					</view>
		...
		<view class="bottomAction">
			...
			<u-grid customStyle="border-top: 1px solid #eee; padding-top: 10px"
				v-if="collectionDetails.status==5 || collectionDetails.status==6" :border="false" col="3">
				<u-grid-item @click="editCollection">
					<view class="grid-img">
						<image class="logo-max" src="/static/img/workbench/icon_planedit.png"></image>
					</view>
					<text class="grid-text" style="color:#606266;font-size: 12px;">编辑回款单</text>
				</u-grid-item>
				<u-grid-item @click="submitCollection">
					<view class="grid-img">
				...
				<u-grid-item @click="deleteRecord">
					...
					<text class="grid-text" style="color:#606266;font-size: 12px;">删除回款单</text>
				</u-grid-item>
			</u-grid>

			<!-- 待审批 -->
			<u-grid customStyle="border-top: 1px solid #eee; padding-top: 10px"
				v-if="collectionDetails.status==1 || collectionDetails.status==2" :border="false" col="1">
			...
			<!-- 已通过, 不可操作 -->
			<!-- <u-grid v-if="collectionDetails.status==3" :border="false" col="1">
				<u-grid-item @click="deleteRecord">
					<view class="grid-img">
						<image class="logo-max" src="/static/img/workbench/icon_planedit.png"></image>
					</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			console.log(option)
			let detail = uni.getStorageSync("collectionItem");
			this.collectionItemId = detail.id
		...
		methods: {
			...
			getData() {
				...
				let data = '?id=' + this.collectionItemId
				getCollectionItem(data, res => {
					if (res.code == 200 && res.success) {
						this.collectionDetails = res.result
						uni.setStorageSync("editItem", this.collectionDetails)
						console.log(this.collectionDetails);
						// if (this.collectionDetails.status == 6 || this.collectionDetails.status == 5) {
			...
			editCollection() {
				...
				uni.navigateTo({
					url: "/pages/workbench/accountingManagement/cashtransferSchemes/addCollectionRecord?editCollectionId=" +
						this.collectionItemId
				})
			},
			...
			deleteRecord() {
				let data = '?id=' + this.collectionItemId
				deleteCollectionRecord(data, res => {
					if (res.code == 200 && res.success) {
