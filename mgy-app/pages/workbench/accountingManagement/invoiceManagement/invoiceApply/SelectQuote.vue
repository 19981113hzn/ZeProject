<template>
	...
	<view class="content" :style="{ 'padding-top': statusBarHeight + 'px' }">
		<u-navbar :title="isQuota? '产品报价':'选择报价'" bgColor="#fff" @leftClick="leftClick">
		...
		<!-- 有数据时 -->
		<view v-if="lstsumExpense.length > 0">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			console.log(this.lstsumExpense);
		...
		onShow() {
			...
			if (this.id) {
				...
				getproductQuoteList(catedata, res => {
					if (res.code == 200 && res.success) {
						console.log(res)
						if (this.pageNo == 1) {
							this.lstsumExpense = res.result.records;
		...
		methods: {
			...
			nextStep() {
				...
				// if (this.isQuota) { //跳转产品-确认报价
					// uni.navigateTo({
					// 	url: "/pages/workbench/operationManagement/opportunityManagement/confirmQuotation?bizOpptyId=" +
					// 		this.bizOpptyId + '&type=' + this.type
					// })
				// } else {
					// 提交关联产品
				// 	console.log('选择完毕');
				// 	uni.navigateTo({
				// 		url: '../../workbench/operationManagement/opportunityManagement/confirmQuotation?isAddOpp=' +
				// 			this.isAddOpp + '&type=' + this.type + "&returnNumber=2"
				// 	})
				// }

				uni.navigateTo({
						url: "/pages/workbench/accountingManagement/invoiceManagement/invoiceApply/relatedProduct"
					})
			},
			groupChange(e) {
				this.checkboxValue1 = e
				e.forEach(item => {
					let selectItem = this.lstsumExpense[item]
