<template>
	<view class="content" :style="{ 'padding-top': statusBarHeight + 'px' }">
		<u-navbar title="付款计划" bgColor="#fff" @leftClick="leftClick" @rightClick="rightClick">
		...
		<view class="card" v-for="(item, index) in lstPayment" :key="index">
			<view class="card-pa" @click="gotoPaymentDetail(item)">
				...
				<u--form labelWidth="80">
					...
					<u-form-item label="合同归属人">
						<text>{{item.contractUserName != null ? item.contractUserName : '李晓明'}}</text>
					</u-form-item>
				</u--form>
		...
		<u-empty v-if="lstPayment.length==0 && isLoading" mode="data"
			icon="http://cdn.uviewui.com/uview/empty/data.png">
		</u-empty>
		<u-loadmore v-if="lstPayment.length>0" :status="status" :loading-text="loadingText"
		...
		methods: {
			...
			getPayment() {
				...
				let data = "?pageNo=" + this.pageNo //页码
					...
					"&pageSize=" + "10" //页码大小
					+
					"&keyword=" + this.selectInfo.keyword //搜素关键词
					+
					"&status=" + this.selectInfo.status //付款状态
					+
					"&listType=" + this.selectInfo.listType // 付款计划分类   0全部 1我的 2下属的
					+
					"&planId=" + this.selectInfo.planId +
					"&selections=" + this.selectInfo.selections +
					"&orgId=" + this.selectInfo.orgId +
					"&userIdList=" + this.selectInfo.userIdList //合同所属人员
					+
					"&deptIdList=" + this.selectInfo.deptIdList

				console.log('筛选条件', data)
				getPaymentPlanList(data, res => {
					if (res.code == 200 && res.success) {
						console.log(res)
						if (this.pageNo == 1) {
							this.lstPayment = res.result.records;
						} else {
							this.lstPayment = this.lstPayment.concat(res.result.records)
						}
			...
			gotoPaymentDetail(item) {
				console.log(item);
				uni.setStorageSync('paymentDetails', item)
				uni.navigateTo({
			...
			rightClick() {
				uni.navigateTo({
					url: "/pages/workbench/accountingManagement/paymentSchemes/addPaymentPlan?returnPageNumber=1"
				})
			}
		},
	}
