<template>
	<view class="content" :style="{ 'padding-top': statusBarHeight + 'px' }">
		<u-navbar title="竞争对手"  bgColor="#fff" @leftClick="leftClick" @rightClick="rightClick">
				...
				<view class="card_top">
					...
					<view class="contentBox">
						...
						<view class="desBox">
							...
							<view :class="item.competitivePower == 1 ? 'company_des':item.competitivePower == 2 ?'company_des_B':item.competitivePower == 3 ? 'company_des_C':''" v-if="item.competitivePower_dictText">{{'竞争能力' + item.competitivePower_dictText}}</view>
...
<script>
	...
	import {getDictItemsByCode,getUserListByUserId,getCompetitorList} from "@/util/interface.js"
	export default {
		data() {
			return {
				topheight:0,
				...
				lstScreen:[
					{
						...
						lst:[
							// {name:"全部",value:""},
							// {name:"A(优质竞争对手)",value:"1"},
							// {name:"B(普通竞争对手)",value:"2"}
						]
					},
					{
		...
		onLoad(){
			...
			this.getDictItems('competitive_power');
		},
		onReachBottom() {
		...
		onShow() {
		    ...
		    uni.$once('result', res => {
			    for(let i=0;i<res.length;i++){
					let item=res[i];
					if(item.title=="竞争对手级别"){
						this.selectInfo.competitivePower=item.defaultSelectedIndex;
					}
					if(item.title=="操作时间"){
		...
		methods: {
			...
			getSupplier(){
				...
				+"&pageSize="+"5"//页码大小
				+"&name="+this.selectInfo.name//竞争对手名称
				+"&competitivePower="+this.selectInfo.competitivePower//竞争对手级别
			...
			getDictItems(type){
				...
				getDictItemsByCode(data,res=>{
					if(res.code==200 && res.success){
						...
						if(type != 'supplier_level' && type != 'competitive_power'){
						...
						for(let i=0;i<this.lstScreen.length;i++){
							if(this.lstScreen[i].title=="竞争对手级别"){
								this.lstScreen[i].lst=lst;
								if(type == 'competitive_power'){
									this.lstScreen[i].lst.unshift({name:"全部",value:""})
								}
							}
						}
						uni.setStorageSync(type,JSON.stringify(res.result))
			...
			gotoFeeDetail(item){
				uni.navigateTo({
					url: "/pages/competitor/detail?id=" + item.id
				})
			},
			clickScreen(){
				if(!this.firstTime){
			...
			rightClick(){
				uni.navigateTo({
					url: "/pages/competitor/addCompetitor?comefrom=" + 'list'
				})
			}
		},
		// 按钮监听函数
