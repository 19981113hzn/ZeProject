		...
		<view v-else>
			<u--form :model="itemInfo" ref="form1" labelPosition="left" labelWidth="105" id="myForm">
				<view class="top-height">
					...
					<u-form-item label="竞争对手能力" prop="competitivePower" borderBottom ref="item1"
						@click="clickExpense('competitive_power'); hideKeyboard()">
						<u--input v-model="competitivePowerName" border="none" placeholder="请选择" clearable disabled
				...
				<view class="top-height">
					...
					<u--textarea v-model="itemInfo.companyIntroduction" border="none" placeholder="请输入公司介绍"
						maxlength="300" v-if="navTitle != '编辑竞争对手'"></u--textarea>
					<u-read-more :toggle="true" showHeight="100" closeText="展开" v-if="navTitle == '编辑竞争对手'">
				...
				<view class="top-height" v-if="!isUp">
					...
					<u-form-item label="产品介绍" prop="competitiveProduction" ref="item1"></u-form-item>
					<u--textarea v-model="itemInfo.competitiveProduction" border="none" placeholder="请输入竞争产品的介绍"
						maxlength="300" v-if="navTitle != '编辑竞争对手'"></u--textarea>
					<u-read-more :toggle="true" showHeight="200" closeText="展开" v-if="navTitle == '编辑竞争对手'">
						<rich-text :nodes="itemInfo.competitiveProduction" @load="load" class="richTextContent">
						</rich-text>
					</u-read-more>
				</view>

				<view class="top-height" v-if="!isUp">
					<u-form-item label="竞争对手优势" prop="advantage" ref="item1"></u-form-item>
					<u--textarea v-model="itemInfo.advantage" border="none" placeholder="请输入对手优势" maxlength="300"
						v-if="navTitle != '编辑竞争对手'"></u--textarea>
					<u-read-more :toggle="true" showHeight="200" closeText="展开" v-if="navTitle == '编辑竞争对手'">
						<rich-text :nodes="itemInfo.advantage" class="richTextContent"></rich-text>
					</u-read-more>
				</view>
				<view class="top-height" v-if="!isUp">
					<u-form-item label="竞争对手劣势" prop="disadvantage" ref="item1"></u-form-item>
				...
				<view class="top-height" v-if="!isUp">
					<u-form-item label="应对策略" prop="copingStrategies" ref="item1"></u-form-item>
					<u--textarea v-model="itemInfo.copingStrategies" border="none" placeholder="请输入应对策略" maxlength="300"
						v-if="navTitle != '编辑竞争对手'"></u--textarea>
					<u-read-more :toggle="true" showHeight="200" closeText="展开" v-if="navTitle == '编辑竞争对手'">
						<rich-text :nodes="itemInfo.copingStrategies" class="richTextContent"></rich-text>
					</u-read-more>
				</view>
				
				<!-- <view class="top-height" v-if="!isUp">
					<u-form-item label="应对策略" prop="copingStrategies" ref="item1"></u-form-item>
					<u--textarea v-model="itemInfo.copingStrategies" border="none" placeholder="请输入应对策略" maxlength="300" v-if="navTitle != '编辑竞争对手'"></u--textarea>
					<u-read-more :toggle="true" showHeight="200" closeText="展开" v-if="navTitle == '编辑竞争对手'">
							<rich-text :nodes="itemInfo.copingStrategies"  class="richTextContent"></rich-text>
						</u-read-more>
				</view> -->
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.competitive_power.push(getApp().globalData.competitive_power)
		...
		methods: {
			...
			getInfo(item) {
				...
				this.competitive_power[0].forEach(v => {
					if (v.value == this.itemInfo.competitivePower) {
						this.competitivePowerName = v.label
					}
				})
			},
			focusEven() {
			...
			clickExpense(type) {
				...
				switch (type) {
					...
					case 'competitive_power':
						this.lstExpense_type = this.competitive_power;
						this.showPicker = true;
						break;
			...
			confirmExpense_type(e) {
				...
				switch (type) {
					...
					case 'competitive_power':
						this.itemInfo.competitivePower = e.value[0].value + '';
						this.competitivePowerName = e.value[0].label;
						break;
			...
			saveCost(){
				if(this.validateField()){
					...
					this.itemInfo.isMicroEnt = this.itemInfo.isMicroEntStatus?'1':'0'
						...
						if (this.itemInfo.disadvantage) this.itemInfo.disadvantage =
							`<p>${this.itemInfo.disadvantage}</p>`;
						if (this.itemInfo.copingStrategies) this.itemInfo.copingStrategies =
							`<p>${this.itemInfo.copingStrategies}</p>`;
