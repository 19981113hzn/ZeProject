<template>
	<view id="detailBox">
		...
		<!-- <view class="navbarBoxTop" v-show="showNavbar">
			<view class="back" @click="leftClick"><image class="logo_back" src="/static/img/assist/icon_fh.png"></image></view>
			<view class="detailTitle">供应商详情</view>
			<view class="back" @click="rightClick">
				<image class="user-head_more" src="/static/img/assist/detail_more.png"></image>
			</view>
		</view> -->
		
		<u-navbar class="navbar" :title="reimburseDetails.name ||'供应商详情'" :border="false" safeAreaInsetTop fixed placeholder @leftClick="leftClick"
		...
		<view v-else class="contentBigBox">
				...
				<view class="card_top">
					...
					<view class="contentBox">
						...
						<view class="desBox">
							...
							<view :class="reimburseDetails.supplierLevel == 'A'? 'company_des':'company_des_B'" v-if="supplierLevelName">{{supplierLevelName}}</view>
					...
					<view class="card">
						<view class="card-pa">
							...
							<u--form labelWidth="95">
								...
								<u-form-item label="供应商级别">
									<text>{{supplierLevelName || '-' }}</text>
								</u-form-item>
								...
								<u-form-item label="详细地址" @click="gotoMap(reimburseDetails.address)">
									<text style="color:#2979ff"><u-icon name="map-fill" color="#2979ff" size="20" style="display: inline-block;" v-if="reimburseDetails.address"></u-icon>{{reimburseDetails.address || '-' }}</text>
								</u-form-item>
						...
						<view class="card-pa product">
							...
							<u--form labelWidth="0">
								<u-read-more :toggle="true" showHeight="70" closeText="展开">
									<rich-text :nodes="reimburseDetails.mainProduction || '-' " @load="load"
			...
			<view v-show="current == 1" id="yyy">
				<view v-if="reimburseDetails.commentList.length > 0">
					<view class="card">
						<view class="card-pa commentList">
							<u--form labelWidth="75">
								<view class="commentListBox" v-for="item in reimburseDetails.commentList" :key="item.id">
	...
	export default {
		...
		methods: {
			...
			getData() {
				...
				let data="/"+this.id;
				getSupplierDetail(data, res => {
					if (res.code == 200 && res.success) {
						this.reimburseDetails = res.result
						// 供应商级别
						this.supplier_level.forEach((v) => {
							if (v.value == this.reimburseDetails.supplierLevel) {
								this.supplierLevelName = v.text
							}
						})
						// 企业类型
			...
			changeTabbar(name) {
				...
				if (name == 2) {
					...
					uni.navigateTo({
						url: "/pages/supplier/addSupplier?comefrom=" + 'detail'
					})
				}
			},
