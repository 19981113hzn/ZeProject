	...
	<view :style="{ 'padding-top': statusBarHeight + 'px' }">
		...
		<view style="flex: auto; height:auto!important;padding: 16px 16px 0;">
			<scroll-view class="scrool-more" :style="{'top': (statusBarHeight+70) +'px','bottom':'104px' }"
				...
				<u-empty v-if="lstUser.length==0" mode="data" icon="http://cdn.uviewui.com/uview/empty/data.png">
				</u-empty>
			</scroll-view>
		</view>
		<view class="foot-height">
			...
			<view style="padding:12px 16px 16px">
				<u-button color="rgba(0, 114, 255, 1)" shape="circle" text="确认" @click="confirmSelect"></u-button>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			let total = isEmpty(option.total) ? false : option.total;
			this.isMutiple = isEmpty(option.isMutiple) ? false : option.isMutiple;
			this.navTitle = option.title ? option.title + "(" + total + ")" : "负责人员(" + total + ")"
			this.total = total;
		...
		methods: {
			...
			getResponsible() {
				getUserListByName('', res => {
					...
					// 转移客户的时候需要过滤掉自己
					const records = this.needFilter ? res.result.records.filter(fi => fi.id != userInfo.id) : res
						.result.records;
			...
			selectItem(index) {
				...
				if (this.isMutiple) { //单选
					let arr = [];
					arr.push(this.lstUser[index])
					this.selectLst = arr;
					this.seeLst = arr;
					//this.lstScreen[index].defaultSelectedIndex=value;
					for (let i = 0; i < this.lstUser.length; i++) {
						if (value != this.lstUser[i].value) {
							this.lstUser[i].isSelected = false;
						} else {
							this.lstUser[i].isSelected = true;
						}
					}
				} else { //多选
