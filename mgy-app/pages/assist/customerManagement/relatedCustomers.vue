<template>
	<view :style="{ 'padding-top': statusBarHeight + 'px' }">
		<u-navbar title="关联客户" bgColor="#fff" @leftClick="leftClick">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			console.log(option);
			this.radiovalue1 =option.customerId || ""
			// if (option.customerIndex) {
			// 	this.radiovalue1 = Number(option.customerIndex)
		...
		methods: {
			...
			groupChange(e) {
				...
				// // this.customerIndex = e
				// this.customerName = this.lstCustomer[e].name
				// this.customerId = this.lstCustomer[e].id
			...
			save() {
				if(!isEmpty(this.radiovalue1)){
					let data={
						...
						customerId:this.radiovalue1
					}
					console.log(data)
					uni.$emit('returnCustomer', data);
					uni.navigateBack({
