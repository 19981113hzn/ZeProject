...
export default {
	...
	methods: {
		...
		handleClick(item, index) {
			...
			if (index > -1 && children.length == 0 || index == -1 && children.length == 0) {
				console.log('111')
				// uni.navigateTo({
				//         url: "./index?id=" + item.id
				//     })
				// getCatePageList(data, res => {
				//         if (res.code == 200 && res.success) {
				//             console.log(res)
				//             if (this.pageNo == 1) {
				//                 this.catePageList = res.result.records;
				//             } else {
				//                 this.catePageList = this.catePageList.concat(res.result.records)
				//             }
