...
class HtmlParser extends Component {
    ...
    renderItems = () => {
        ...
        this.imgLinkArr.forEach((item, i) => {
            ReactDom.render(
                <ImgLink
                    id={`imgLink${i}`}
                    outLink={this.imgLinkArr[i].outLink}
                    imgSrc={this.imgLinkArr[i].imgSrc}
                />,
                document.getElementById(`imgLinkEl-${i}`)
            );
        });
    ...
    handleFilterContent = (content) => {
        ...
        if (content) {
            ...
            // 处理资源协议
            content = content.replace(/src="https:\/\//gi, 'src="//').replace(/src="http:\/\//gi, 'src="//');
            ...
            content = content.replace(videoReg, (match) => {
                console.log(match.split('src="')[1], '第一个', match.split('src="')[2], '第er个', match.split('src="')[2], 'kkkkkk');
                const data = {
                    posterUrl: match.split('poster="')[1]?.split('"')[0] || '',
                    videoUrl: match.split('src="')[2]?.split('"')[0] || ''
                };
                console.log(data.videoUrl, '视频地址');
            ...
            // 整理文章内的带外链的Img标签
            const imgLinkReg = /<p[^>]*><a data="outlink"[^>]*href=['"]([^'""]+)?[^>]*><img[^>]*><\/a><\/p>/g;
            content = content.replace(imgLinkReg, (match) => {
                const data = {
                    outLink: match.split('href="')[1]?.split('"')[0] || '',
                    imgSrc: match.split('src="')[1]?.split('"')[0] || ''
                };
                this.imgLinkArr.push(data);
                return `<div id="imgLinkEl-${this.imgLinkArr.length - 1}" class="${styles['img-link-shell']}"></div>`;
            });
        }
